...
class Dock(object):
    ...
    def read_settings(self):
        ...
        if self.settings.get_boolean("first-run") is True:
            ...
            if xml_settings[0] is True:
                ...
                self.click_action = xml_settings[18]
                self.theme = xml_settings[19]
    ...
    def setup_menu(self):
        ...
        menu_xml = ""
        while shortcut_action_no <= self.max_num_actions:
            menu_xml += '<menuitem name="df_shortcut_%d_action" action="df_shortcut_%d_action"/><separator/>' % (shortcut_action_no, shortcut_action_no)
            shortcut_action_no += 1
